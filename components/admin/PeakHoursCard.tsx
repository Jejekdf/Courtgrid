...
export default function PeakHoursCard({
  ...
  return (
    <motion.div
      ...
      className="bg-white border border-zinc-200 rounded-xl shadow-sm overflow-hidden flex flex-col justify-between"
    >
      <div className="px-6 py-5 border-b border-zinc-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div className="flex flex-col gap-1">
          <h3 className="text-lg font-medium tracking-tight text-zinc-950 text-balance">
            {t("peakHoursTitle")}
          </h3>
        ...
        {hasData && peakHour && (
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-zinc-100 border border-zinc-200 text-xs font-mono font-semibold text-zinc-900 self-start sm:self-auto shrink-0">
      ...
      <div className="px-6 py-6 flex-1 flex flex-col justify-center">
        {hasData ? (
          ...
          <div className="h-[240px] flex items-center justify-center text-sm text-zinc-400 font-mono">
            {t("noPeakData")}
          </div>
        )}
      </div>

      <div className="px-6 py-3.5 bg-zinc-50 border-t border-zinc-200 flex items-start gap-2.5 text-xs text-zinc-600">
        <TrendingUp className="size-4 text-zinc-900 shrink-0 mt-0.5" />
